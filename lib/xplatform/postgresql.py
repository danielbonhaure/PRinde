...
def read_connection_parameters():
    ...
    conn.user = raw_input("Ingrese el usuario de la Base de Datos [postgres]: ")
    if not conn.user:
        conn.user = "postgres"
    ...
    # Escribimos en la variable de entorno la contraseña de postgres para evitar pasarla en plaintext al proceso.
    os.environ["PGPASSWORD"] = pwd

    return conn


def run_script(postgresql, user, db_name, script_path, script_parameter=None):
    # Armamos el comando que le vamos a mandar a la consola.
    cmd = "\"%s\" -p10521 -U %s -h localhost " % (postgresql, user)
