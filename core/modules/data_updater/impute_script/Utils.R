...
Gauss.Kruger.coordinates <- function(df) {
    spTransform(SpatialPointsDataFrame(cbind(x = df[,"lon_dec"], y = df[,"lat_dec"]), data=df,
                                       proj4string = CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"), bbox = NULL),
                CRS("+proj=tmerc +lat_0=-90 +lon_0=-60 +k=1 +x_0=5500000 +y_0=0 +ellps=intl +twogs84=-148,136,90,0,0,0,0 +units=m +no_defs"))
}
...
insert_and_check <- function(db_connection, insert_query, expected_results) {
    rs <- dbSendQuery(db_connection, insert_query)
    rowsAffected <- dbGetInfo(rs, what = "rowsAffected")
